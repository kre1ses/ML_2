...
class WordAligner(BaseAligner):
    ...
    def _e_step(self, parallel_corpus: List[TokenizedSentencePair]) -> List[np.array]:
        ...
        for sentence_pair in parallel_corpus:
            src, tgt = sentence_pair.source_tokens, sentence_pair.target_tokens

            posterior = self.translation_probs[np.ix_(src, tgt)]
            devider = posterior.sum(axis=0, keepdims=True)
            posteriors.append(posterior / devider)
    ...
    def _compute_elbo(self, parallel_corpus: List[TokenizedSentencePair], posteriors: List[np.array]) -> float:
        ...
        for sentence_pair, posterior in zip(parallel_corpus, posteriors):
            src, tgt = sentence_pair.source_tokens, sentence_pair.target_tokens
            log_p_q = np.log((self.translation_probs[np.ix_(src, tgt)] + 1e-9) / (posterior * len(src) + 1e-9))
            elbo += np.sum(posterior * log_p_q)
        return elbo
    ...
    def _m_step(self, parallel_corpus: List[TokenizedSentencePair], posteriors: List[np.array]):
        ...
        for sentence_pair, posterior in zip(parallel_corpus, posteriors):
            ...
            np.add.at(self.translation_probs, np.ix_(src, tgt), posterior)
